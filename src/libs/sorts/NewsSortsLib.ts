...
const NewsSortsLib = (() => {
  ...
  function byMain(order?: Orders): SortModel {
    return SortModel.Create(News, ClassHelper.GetPropertyName(News).main, order ? order : Orders.Desc, 'Сначала главная', true);
  }
  ...
  function byTitle(order?: Orders): SortModel {
    return SortModel.Create(News, ClassHelper.GetPropertyName(News).title, order ? order : Orders.Asc, 'По названию');
  }

  function byCreatedAt(order?: Orders): SortModel {
    return SortModel.Create(News, ClassHelper.GetPropertyName(News).createdAt, order ? order : Orders.Desc, 'По дате создания');
  }
