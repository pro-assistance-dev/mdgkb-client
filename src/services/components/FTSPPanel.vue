<template>
  ...
  <PButton skin="text" type="success" text="Добавить текущий фильтр в список фильтров" @click="openModal" v-if="ftsp.f.length > 0"/>
  <PButton skin="text" type="del" text="Удалить выбранный фильтр из списка фильтров" @click="remove" v-if="curFTSP" />
  <ModalWindow v-if="modalOpened" :show="modalOpened">
    <el-input v-model="ftspName" placeholder="Введите название" />
    <PButton skin="profile" type="primary" text="Сохранить" @click="save" />
  </ModalWindow>
</template>
