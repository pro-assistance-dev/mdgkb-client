...
export default class User implements IUser {
  ...
  static validatePhone = (_: unknown, value: string, user: CallableFunction): void => {
    if (User.getPhoneRegExp().test(value) || !value) {
      user();
    } else {
      user(new Error('Пожалуйста, введите корректный номер телефона'));
    }
  };
}
