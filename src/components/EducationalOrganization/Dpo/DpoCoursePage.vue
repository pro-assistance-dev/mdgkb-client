<template>
  <div v-if="mounted" class="medical-profile-page-container">
    <div class="side-container hidden-md-and-down">
      ...
      <div class="side-item">
        <div class="card-item">
          <h4 class="card-item-title">Длительность курса: {{ dpoCourse.hours }}ч.</h4>
        </div>
      </div>
      <div class="side-item">
        <div class="card-item">
          <h4 class="card-item-title">Курс начинается {{ $dateFormatRu(dpoCourse.start, true, false) }}</h4>
        </div>
      </div>
    </div>
    <div class="card-item">
...
export default defineComponent({
  name: 'DpoCoursePage',
  components: { SharesBlock, HelpProfileIcon, BaseIcon },
  ...
  setup() {
    ...
    const getUrl = (): string => {
      ...
      return `${host}/dpo-courses/${route.params['id']}`;
    };
    // TODO убрать
    const svgDummy =
    ...
    return { chooseRandomBrandColor, dpoCourse, getUrl, svgDummy, mounted };
  },
});
</script>
