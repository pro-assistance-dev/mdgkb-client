<template>
  <el-form-item label="ВУЗ" prop="diploma.universityName"
    :rules="[{ required: true, message: 'Необходимо указать ВУЗ', trigger: 'blur' }]">
    <el-input v-model="diploma.universityName" placeholder="ВУЗ"></el-input>
  </el-form-item>
  <el-form-item label="Дата окончания" prop="diploma.universityEndDate"
    :rules="[{ required: true, message: 'Необходимо указать дату окончания', trigger: 'change' }]">
    <DatePicker v-model="diploma.universityEndDate" />
  </el-form-item>
  <img :src="DiplomaExample" alt="diploma-example" />
  <el-form-item label="Серия" prop="diploma.series"
    :rules="[{ required: true, message: 'Необходимо указать серию', trigger: 'blur' }]">
    <el-input v-model="diploma.series" placeholder="Серия"
      @input="(e) => (diploma.series = e.replace(/\D+/g, ''))"></el-input>
  </el-form-item>
  <el-form-item label="Номер" prop="diploma.number"
    :rules="[{ required: true, message: 'Необходимо указать номер', trigger: 'blur' }]">
    <!-- <div style="display: flex"> -->
    <el-input v-model="diploma.number" placeholder="Номер"
      @input="(e) => (diploma.number = e.replace(/\D+/g, ''))"></el-input>
    <!-- <el-button style="margin-left: 10px" icon="el-icon-document-copy" @click="clickCopyHandler"></el-button> -->
    <!-- </div> -->
  </el-form-item>
  <el-form-item label="Когда выдан" prop="diploma.date"
    :rules="[{ required: true, message: 'Необходимо указать дату выдачи', trigger: 'change' }]">
    <DatePicker v-model="diploma.date" />
  </el-form-item>
  <el-form-item label="Специальность по диплому КОД (например, 31.05.02)" prop="diploma.specialityCode"
  ...
  <el-form-item label="Расшифровка специальности (например, педиатрия)" prop="diploma.speciality"
