  ...
  <div v-if="mounted" class="form-container">
    ...
    <div class="card-item">
      <el-form ref="form" :model="visitsApplication" label-position="top" :rules="rules">
        <UserForm :validate-email="false" :form="visitsApplication.formValue"
        ...
        <el-form-item>
          <template #label>
            <div @click.prevent>
              ...
              <el-button style="margin-left: 10px" size="mini" type="success"
                @click="visitsApplication.addVisit()">Добавить</el-button>
            </div>
          </template>
          <div v-for="(item, i) in visitsApplication.visits" :key="i" style="margin-bottom: 10px">
            ...
            <el-button v-if="visitsApplication.visits.length > 1" style="margin-left: 10px" size="mini"
              icon="el-icon-close" type="danger" @click="visitsApplication.removeVisit(i)" />
          </div>
        </el-form-item>
        <el-form-item>
          <template #label>
            <div @click.prevent>
              ...
              <el-button v-if="!visitsApplication.withCar" style="margin-left: 10px" size="mini" type="success"
                @click="visitsApplication.changeWithCar(true)">
                Добавить данные об автомобиле
              </el-button>
              <el-button v-else style="margin-left: 10px" size="mini" type="danger"
                @click="visitsApplication.changeWithCar(false)">
          ...
          <div v-if="visitsApplication.withCar">
            <el-form-item label="Номер автомобиля" :rules="rules.userCarNumber" prop="formValue.user.human.carNumber">
              <el-input v-model="visitsApplication.formValue.user.human.carNumber"
                placeholder="Номер автомобиля"></el-input>
            </el-form-item>
            <el-form-item label="Марка автомобиля" :rules="rules.userCarModel" prop="formValue.user.human.carModel">
              <el-input v-model="visitsApplication.formValue.user.human.carModel"
                placeholder="Марка автомобиля"></el-input>
            </el-form-item>
          </div>
        </el-form-item>
